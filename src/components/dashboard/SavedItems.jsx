...
export const SavedItems = ({ likes: initialLikes, bookmarks: initialBookmarks, onUpdate }) => {
    ...
    const fetchSavedItems = useCallback(async () => {
        try {
            setLoading(true);
            const response = await apiCall('/profile/saved');

            // Transform to expected state structure
            const customFolders = response.folders || [];
            const allFolder = {
                id: 'default',
                name: 'الكل',
                icon: '📁',
            ...
            setSavedItems({
                ...
                folders: [allFolder, ...customFolders]
            });
        } catch (error) {
    ...
    const handleRemoveItem = async (item) => {
        try {
            ...
            const section = sectionMap[activeTab] || 'bookmarks';
            const stateKey = activeTab === 'reading' ? 'readingList' : activeTab;

            await apiCall('/profile/saved', {
                method: 'POST',
                body: JSON.stringify({
                    action: 'remove',
                    itemId: item.item_id,
            ...
            setSavedItems(prev => ({
                ...prev,
                [stateKey]: prev[stateKey].filter(i =>
    ...
    const createFolder = async () => {
        ...
        try {
            const response = await apiCall('/profile/folders', {
                method: 'POST',
            ...
            setSavedItems(prev => ({
                ...prev,
                folders: [...prev.folders, { ...response, count: 0 }]
            }));
    ...
    const deleteFolder = async (folderId, e) => {
        ...
        if (!window.confirm('هل أنت متأكد من حذف هذا المجلد؟ سيتم نقل المحتويات إلى القائمة الرئيسية.')) return;

        try {
            await apiCall(`/profile/folders/${folderId}`, {
                method: 'DELETE'
            });
            ...
            // Refresh items to update their folder_id references (optional, for UI consistency)
            fetchSavedItems();
        } catch (error) {
        ...
        try {
            await apiCall('/profile/saved', {
                ...
                body: JSON.stringify({
                    ...
                    itemType: item.item_type,
                    folderId: folderId,
                    section: 'bookmarks' // moving implies bookmarks
                })
            });
            ...
            {activeTab === 'bookmarks' && (
                ...
                >
                    {savedItems.folders.map(folder => (
                        <div key={folder.id} className="flex items-center bg-white/5 rounded-lg overflow-hidden group/folder-item">
                            <button
                                onClick={() => setSelectedFolder(folder.id === selectedFolder ? null : folder.id)}
                                className={`flex items-center gap-2 px-3 py-2 font-bold text-sm whitespace-nowrap transition-all ${selectedFolder === folder.id
                            ...
                            >
                                ...
                                <span className={`px-1.5 py-0.5 rounded text-xs ${selectedFolder === folder.id ? 'bg-white/20' : 'bg-white/10'
                                    }`}>
                                    {folder.count || 0}
                                </span>
                            </button>
                            {folder.id !== 'default' && (
                                <button
                                    ...
                                    className="p-2 border-r border-white/10 text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-all opacity-0 group-hover/folder-item:opacity-100"
                                    title="حذف المجلد"
                                >
                        ...
                        {filteredItems.map((item, index) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={`group relative bg-gradient-to-r from-gray-900/80 to-gray-800/80 border border-white/10 rounded-xl overflow-hidden hover:border-blue-500/30 transition-all ${viewMode === 'list' ? 'flex items-center p-4' : 'p-5'
                                        }`}
                                >
                                    {/* Item Icon */}
                                    ...
                                    {/* Open Button */}
                                    <Link
                                        to={item.item_type === 'track' ? `/knowledge/${item.item_id}` : `/knowledge/${item.item_type}s/${item.item_id}`}
                ...
                >
                    ...
                    >
                        ...
                        <p className="text-gray-400 mb-6">
                            هل أنت متأكد من حذف "{itemToDelete.title || 'هذا العنصر'}" من {
                                activeTab === 'bookmarks' ? 'المحفوظات' :
                                    activeTab === 'likes' ? 'الإعجابات' : 'قائمة القراءة'
                            }؟
                        </p>
                        <div className="flex items-center gap-3">
                            <button
