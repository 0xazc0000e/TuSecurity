...
export const MyLearning = ({ enrollments, onProgressUpdate }) => {
    ...
    const fetchLearningData = async () => {
        try {
            ...
            const [tracksResponse, statsResponse] = await Promise.all([
                apiCall('/profile/learning-path'),
                apiCall('/profile/stats')
            ]);

            // Calculate overall progress
            const tracks = tracksResponse || [];
            const activeTracks = tracks.filter(t => t.progress > 0 && t.progress < 100);

            // Derive in-progress lessons from tracks
            const inProgress = activeTracks.map(track => ({
                id: track.current_lesson_id,
                title: track.current_lesson_title || 'متابعة الدرس',
                track_title: track.title,
                course_title: 'المسار الحالي', // We don't have course title in track summary easily, generic is fine
                progress: track.progress // Use track progress as proxy or fetch specific lesson progress if needed
            })).filter(l => l.id);
            ...
            const completedLessonsCount = statsResponse.completedLessons || 0;
            // Create a dummy array for length checks if we don't change JSX
            // Or better, just store the count and update JSX.
            // For minimal JSX changes, we'll store specific keys.

            setLearningData({
                enrolledTracks: tracks,
                inProgressLessons: inProgress,
                completedLessonsCount: completedLessonsCount, // New key
                completedLessons: new Array(completedLessonsCount).fill({}), // Backward compatibility for .length checks
                overallProgress,
                totalLearningTime: Math.floor(completedLessonsCount * 45), // Approximate 45 mins per lesson
                streakDays: statsResponse.currentStreak || 0,
                lastActivity: null // We could fetch from /profile/activity if needed
            });
        } catch (error) {
    ...
    const continueLesson = async (lessonId) => {
        // Navigation is handled by Link, this just logs
        // We could log access here if we had an endpoint
    };

    const getFilteredEnrollments = () => {
        if (activeFilter === 'all') return learningData.enrolledTracks;
        if (activeFilter === 'completed') {
            return learningData.enrolledTracks.filter(track => track.progress === 100);
        }
        if (activeFilter === 'in-progress') {
            return learningData.enrolledTracks.filter(track => track.progress > 0 && track.progress < 100);
        }
        ...
        <div className="space-y-6">
                ...
                <div className="flex items-center justify-between mt-4 text-sm">
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2 text-gray-400">
                            ...
                            <span>{learningData.completedLessonsCount} درس مكتمل</span>
                        </div>
                        <div className="flex items-center gap-2 text-gray-400">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2">
                {[
                    ...
                    { id: 'in-progress', label: 'قيد التقدم', count: learningData.enrolledTracks.filter(t => t.progress > 0 && t.progress < 100).length },
                    { id: 'completed', label: 'مكتمل', count: learningData.enrolledTracks.filter(t => t.progress === 100).length }
                ].map(filter => (
                    <button
                        key={filter.id}
                        onClick={() => setActiveFilter(filter.id)}
                        className={`px-4 py-2 rounded-lg font-bold text-sm whitespace-nowrap transition-all ${activeFilter === filter.id
                                ? 'bg-green-500 text-white'
                                : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10'
                            }`}
                    >
                        {filter.label}
                        <span className={`mr-2 px-2 py-0.5 rounded-full text-xs ${activeFilter === filter.id ? 'bg-white/20' : 'bg-white/10'
                            }`}>
                            {filter.count}
                        </span>
                    </button>
                    ...
                    {filteredEnrollments.map((track, index) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={`relative overflow-hidden rounded-xl p-5 border transition-all cursor-pointer group ${isCompleted
                            ...
                            >
                                ...
                                <div className="relative z-10">
                                    ...
                                    <Link
                                        ...
                                        className={`flex items-center justify-center gap-2 w-full py-2.5 rounded-lg font-bold text-sm transition-all ${isCompleted
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-gray-500 text-xs">{lesson.track_title}</p>
                                    </div>
                                    <div className="flex items-center gap-3">
            ...
            {learningData.completedLessonsCount > 0 && (
                <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="mt-8"
                        ...
                        {[
                            {
                                icon: BookOpen,
                                label: 'أول درس',
                                achieved: learningData.completedLessonsCount >= 1,
                                color: 'green'
                            },
                            {
                                icon: Target,
                                label: '10 دروس',
                                achieved: learningData.completedLessonsCount >= 10,
                                color: 'blue'
                            },
                            {
                                icon: Zap,
                                label: '1000 XP',
                                achieved: false, // Would require XP passed down
                                color: 'yellow'
                            },
                            {
                                icon: Flame,
                            ...
                            <div
                                key={index}
                                className={`p-4 rounded-xl border text-center transition-all ${achievement.achieved
                                        ? `bg-${achievement.color}-500/20 border-${achievement.color}-500/30`
                                        : 'bg-white/5 border-white/10 opacity-50'
                                    }`}
                            >
                                <achievement.icon className={`w-8 h-8 mx-auto mb-2 ${achievement.achieved ? `text-${achievement.color}-400` : 'text-gray-500'
                                    }`} />
