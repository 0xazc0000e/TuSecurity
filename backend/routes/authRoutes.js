...
// router.post('/resend-verification', authController.resendVerification); // Not using code anymore, but links
...
router.get('/google/callback',
    passport.authenticate('google', { failureRedirect: 'http://localhost:5173/login?error=AuthFailed', session: false }),
    authController.socialLoginCallback
);

router.get('/microsoft', passport.authenticate('microsoft', { scope: ['user.read'] }));
router.get('/microsoft/callback',
    passport.authenticate('microsoft', { failureRedirect: 'http://localhost:5173/login?error=AuthFailed', session: false }),
    authController.socialLoginCallback
);
